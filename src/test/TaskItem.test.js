...
describe('TaskItem', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display task details', () => {
      ...
      const targetRow = wrapper.findAll('.detail-row').find(row => 
        row.find('.detail-label').text() === 'Target:'
      )
      expect(targetRow.find('.detail-value').text()).toBe('https://instagram.com/testuser')
    })
  ...
  describe('Actions', () => {
    ...
    it('should show cancelling state when task is cancelling', async () => {
      await wrapper.setProps({ 
        task: { ...mockTask, status: 'cancelling' } 
      })
  ...
  describe('Time Formatting', () => {
    it('should format recent time correctly', () => {
      ...
      expect(wrapper.find('.task-time').text()).toBe('5m ago')
    })
    ...
    it('should format old time correctly', () => {
      ...
      expect(wrapper.find('.task-time').text()).toMatch(/\d{1,2}\/\d{1,2}\/\d{4}/)
    })

    it('should handle missing timestamp', async () => {
      await wrapper.setProps({ 
        task: { ...mockTask, createdAt: null } 
      })
      
      expect(wrapper.find('.task-time').text()).toBe('N/A')
    })
  })
